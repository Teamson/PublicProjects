...
export default class SGConfig {
    ...
    static homeMoreGameType: number = 2                           //首页更多游戏按钮类型  0：抽屉  1：更多好玩  2：抽屉&更多好玩
    static isShowHomeBanner: boolean = true                      //是否展示首页banner
    static isShowShopBanner: boolean = true                      //是否展示商店banner
    ...
    static initConfigData(cb: Function) {
        ...
        if (window['wx'].getLaunchOptionsSync().query.channel) {
            localStorage.setItem('channel', window['wx'].getLaunchOptionsSync().query.channel)
        }
        this.getServerConfig((data: any) => {
            ...
            let channel = window['wx'].getLaunchOptionsSync().query.channel
