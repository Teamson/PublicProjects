...
export default class SGAD {
    ...
    static initBanner() {
        if (!WECHAT || this.front_banner_ids.length <= 0) return;
    ...
    static createVideoAd() {
        if (!WECHAT || this.front_video_ids.length <= 0) return
    ...
    static showVideoAd(finishCB?: Function, cancelCB?: Function, completeCB?: Function, errorFinish = false) {
        if (!WECHAT || this.front_video_ids.length <= 0) {
            finishCB && finishCB();
            cancelCB && cancelCB();
    ...
    private static createFullGrid() {
        if (!WECHAT || this.front_custom_full_ids.length <= 0) { this.isFullGridAdLoaded = true; return }
        let loadCount = 0
    ...
    private static createSideGrid() {
        if (!WECHAT || this.front_custom_side_ids.length <= 0) return
        for (let i = 0; i < 2; i++) {
    ...
    private static createSingleGrid() {
        if (!WECHAT || this.front_custom_single_ids.length <= 0) return
        for (let i = 0; i < 2; i++) {
    ...
    private static createInterstitialAd() {
        if (!WECHAT || this.front_interstitial_ids.length <= 0) return
